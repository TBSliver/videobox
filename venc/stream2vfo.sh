...
if [ -z "$PRES_FILE" ]; then
		...
		bash -x ${ENCPATH}/transcode_2016.sh --audio "${AUDIO}" --camera "${SRCPATH}/${ROOM}cam/${CAM_FILE}" --cameraseek "$CAM_START" --room "$ROOM"  --duration "$DURATION" --title "$TITLE_SANITISED" --speakers "$SPEAKERS_SANITISED" &

else

		bash -x ${ENCPATH}/transcode_2016.sh --audio "${AUDIO}" --camera "${SRCPATH}/${ROOM}cam/${CAM_FILE}" --cameraseek "$CAM_START" --room "$ROOM" --slides "${SRCPATH}/${ROOM}slides/${PRES_FILE}" --slidesseek "$PRES_START"  --duration "$DURATION" --title "$TITLE_SANITISED" --speakers "$SPEAKERS_SANITISED" &
